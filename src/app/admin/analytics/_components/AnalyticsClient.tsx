...
export default function AnalyticsClient({ data: initialData }: AnalyticsClientProps) {
  const { profile, loading } = useProfile()
  const [analytics, setAnalytics] = useState<AnalyticsData | null>(initialData || null)
  const [loadingAnalytics, setLoadingAnalytics] = useState(!initialData)
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const totalRevenue = safeRevenueData.reduce((sum, payment) => sum + (payment?.amount || 0), 0)
      const totalBookings = safeBookingsData.length
      const completedBookings = safeBookingsData.filter(b => b?.status === 'completed').length
      const pendingBookings = safeBookingsData.filter(b => b?.status === 'pending').length
      const cancelledBookings = safeBookingsData.filter(b => b?.status === 'cancelled').length
      const totalCustomers = safeCustomersData.length
      ...
      const dailyRevenue = calculateDailyTrends(safeRevenueData, 'amount')
      const dailyBookings = calculateDailyTrends(safeBookingsData, 'count')

      // Calculate top services (simplified)
      const topServices = calculateTopServices(safeBookingsData)
    ...
    data.forEach(item => {
      ...
      try {
        ...
        if (valueField === 'amount') {
          trends[date] = (trends[date] || 0) + (item.amount || 0)
        } else {
          trends[date] = (trends[date] || 0) + 1
        }
      } catch (error) {
        console.warn('Invalid date in calculateDailyTrends:', item.created_at)
      }
    })
                  ...
                  {(analytics.trends.top_services || []).map((service, index) => (
                    <div key={index} className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-gray-900">{service?.name || 'Unknown Service'}</p>
                          <p className="text-xs text-gray-500">{service?.count || 0} bookings</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-semibold text-gray-900">{formatCurrency(service?.revenue || 0)}</p>
