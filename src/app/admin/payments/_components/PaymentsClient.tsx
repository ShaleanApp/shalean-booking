...
export default function PaymentsClient({ data: initialData }: PaymentsClientProps) {
  const { profile, loading } = useProfile()
  const [payments, setPayments] = useState<PaymentWithBooking[]>(initialData || [])
  const [filteredPayments, setFilteredPayments] = useState<PaymentWithBooking[]>(initialData || [])
  const [loadingPayments, setLoadingPayments] = useState(!initialData)
  ...
  const fetchPayments = async () => {
    try {
      ...
      calculateStats(safePaymentsData)
    } catch (error) {
  ...
  const calculateStats = (paymentsData: PaymentWithBooking[]) => {
    ...
    const pendingPayments = safePaymentsData.filter(p => p?.status === 'pending').length
    const failedPayments = safePaymentsData.filter(p => p?.status === 'failed').length

    const today = new Date().toDateString()
    const todayRevenue = safePaymentsData
